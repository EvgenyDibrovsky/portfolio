...
export default function PortfolioListItem({ image, image_webp, name, onCardClick }) {
  return (
    <li
      onClick={onCardClick}
      className="shadow-sm shadow-black dark:shadow-white group max-h-72 relative flex flex-col justify-between transition-all duration-500 rounded-md overflow-hidden group opacity-0 init-animate-1 "
    >
      <div className="relative flex items-center justify-center cursor-pointer aspect-video">
        ...
        <h3 className="absolute z-10 bg-orange-400 dark:bg-orange-400 px-2 py-1 top-2 left-0 text-white dark:text-black text-sm rounded-r-lg  shadow-lg duration-300 transform -translate-x-full group-hover:translate-x-0">
          {name}
        </h3>
