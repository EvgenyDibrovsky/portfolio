...
export default function PortfolioListItem({ image, name, onCardClick }) {
  return (
    <>
      <AnimateElements>
        <li className="border border-colorBorder dark:border-colorBorderDark group max-h-72 relative flex flex-col justify-between transition-all duration-500 rounded-md overflow-hidden opacity-0 init-animate-1">
          <div onClick={onCardClick} className="relative flex items-center justify-center cursor-pointer">
            <img src={process.env.PUBLIC_URL + image} alt={name} className="w-full max-h-72 object-cover transition-transform duration-200 ease-in-out group-hover:scale-110" />
